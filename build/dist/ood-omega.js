...
angular.module('OODLib').run(['$templateCache', function ($templateCache) {
	$templateCache.put('lib/aside/aside.html', '<sidebar> <ul class="menu"> <li ng-repeat="(k, v) in model.data"><a ng-href="{{v.URL}}" ng-click="model.toggle(k)"><i ng-class="v.icon"></i><span>{{v.name}}</span><i ng-class="css.arrowIcon(model.expansion, k)" class="is-align-right"></i></a> <ul ng-show="model.expanded(k)" class="menu"> <li ng-repeat="subnode in v.subnodes"><a ng-href="{{subnode.URL}}"><i ng-class="subnode.icon"></i><span>{{subnode.name}}</span></a></li> </ul> </li> </ul> </sidebar>');
